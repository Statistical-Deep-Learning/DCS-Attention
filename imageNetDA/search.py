...
class search_loss(nn.Module):
    def __init__(self, alpha, beta, network_loss_func):
        super(search_loss, self).__init__()
        self.alpha = alpha
        self.beta = beta
        self.network_loss_func = network_loss_func

    def forward(self, output, cost, target):
...
def train(epoch, config, model, optimizer, gs_optimizer, scheduler, gs_scheduler, temperature, loss_func, train_loader,
    ...
    for step, (data, targets) in enumerate(train_loader):
        ...
        for data_chunk, target_chunk in zip(data_chunks, target_chunks):
            ...
            output_chunk, cost_chunk = model(data_chunk, temperature)

            outputs.append(output_chunk)


            search_loss_func = search_loss(config.gs_search.alpha, config.gs_search.beta, loss_func)

            loss, network_loss, latency_loss = search_loss_func(output_chunk, cost_chunk, target_chunk)
            ...
            if config.device != 'cpu':
                ...
                if search_network:
                    with apex.amp.scale_loss(loss, gs_optimizer) as scaled_loss:
                        scaled_loss.backward()
...
def validate(epoch, config, model, loss_func, val_loader, logger,
             tensorboard_writer, temperature):
    ...
    with torch.no_grad():
        for step, (data, targets) in enumerate(val_loader):
            ...
            outputs, cost = model(data, temperature)
